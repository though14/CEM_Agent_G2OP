...
if __name__ == "__main__":
    ...
    # env = env_out
    env = grid2op.make("l2rpn_case14_sandbox_train")
    env.reset()
    ...
    gym_env = GymEnv(env)
    gym_env.action_space = DiscreteActSpace(env.action_space, attr_to_keep=act_space_attr_to_keep) 
    ...
    """
    one : prototype
    Two : Prototype with less loss to be finished
    ...
    path_1 = os.path.join(path_name, 'five')
    ...
    if trainend == False:
        ...
        # env_test = env_sand_test
        env_test = grid2op.make('l2rpn_case14_sandbox_test')
        ...
        res_DN = runner_DN.run(nb_episode=5 ,nb_process=1, pbar = True)
